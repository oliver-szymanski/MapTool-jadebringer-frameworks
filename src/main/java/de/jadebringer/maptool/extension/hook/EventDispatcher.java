...
/**
 * Capture the spread events in any possible way and distribute it further.
 *
 * @author naciron
 */
public class EventDispatcher
    implements AppEventListener, PropertyChangeListener, ModelChangeListener, ContainerListener {
  ...
  public void modelChanged(ModelChangeEvent event) {
    ...
    if (Zone.Event.INITIATIVE_LIST_CHANGED.equals(event.getEvent())) {
      ...
      //   but only for token visible and on token layer (1 unqiue function on[multiple]TokenMove is
      // called)
  ...
  private void registerThisToZone() {
    ...
    if (currentZoneRenderer != null) {
      ...
      MapTool.getFrame()
          .getToolbox()
          .setTargetRenderer(new DelegateZoneRenderer(currentZoneRenderer, this));
    }
  }
  ...
  public void componentRemoved(ContainerEvent e) {
    ...
    if (AppState.getCampaignFile() == null) {
      ...
      if (initializingCampaignState == -1) {
        ...
        Event event =
            Event.create()
                .type(EventType.ApplicationEvent.name())
                .subType(EventSubType.ApplicationStarting.name())
                .source(null)
                .target(null)
                .oldValue(null)
                .value(null)
