...
public class MapFunctions extends ExtensionFunction {
  ...
  private static final String EQUALS = "=";
	...
	public Object center(Parser parser, String functionName, List<Object> parameters) throws ParserException {
	...
	public Object extent(Parser parser, String functionName, List<Object> parameters) throws ParserException {
		ZoneRenderer renderer = MapTool.getFrame().getCurrentZoneRenderer();
		Rectangle rectangle = renderer.zoneExtents(renderer.getPlayerView());
		MapTool.getFrame().getCurrentZoneRenderer().enforceView(rectangle.x, rectangle.y, 1.0, rectangle.width, rectangle.height);
		center(parser, functionName, parameters);
		return BigDecimal.ONE;
	}
	
	public Object getExtent(Parser parser, String functionName, List<Object> parameters) throws ParserException {
		ZoneRenderer renderer = MapTool.getFrame().getCurrentZoneRenderer();
		Rectangle rectangle = renderer.zoneExtents(renderer.getPlayerView());
    ...
    if (!pixels) {
      ...
      return createBoundsAsJSON(rectangle.x, rectangle.y, rectangle.width + rectangle.x, rectangle.height + rectangle.y);
    } else {
      return createBoundsAsStringProps(delim, rectangle.x, rectangle.y, rectangle.width + rectangle.x, rectangle.height + rectangle.y);
    }
	}
